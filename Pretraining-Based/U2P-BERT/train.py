...
def parse_exmp(serial_exmp):
    ...
    text_a_id = input_data["text_a_id"]
    text_b_id = input_data['text_b_id']
    input_ids = input_data["input_ids"]
    input_mask = input_data["input_mask"]
    sids = input_data["segment_ids"]
    labels = input_data['label_ids']
    return text_a_id, text_b_id, input_ids, input_mask, sids, labels


def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, labels, text_a_id, text_b_id,
                 num_labels, use_one_hot_embeddings):
  """Creates a classification model."""

  # print(input_ids.get_shape())    # [batch_size, max_sentence_a_num * max_sentence_b_num * (max_sentence_a_len + max_sentence_b_len)]
  input_ids = tf.reshape(input_ids, [-1, (max_sentence_a_len + max_sentence_b_len)])
  input_mask = tf.reshape(input_mask, [-1, (max_sentence_a_len + max_sentence_b_len)])
  segment_ids = tf.reshape(segment_ids, [-1, (max_sentence_a_len + max_sentence_b_len)])
  # print(input_ids.get_shape())    # [batch_size * max_sentence_a_num * max_sentence_b_num, (max_sentence_a_len + max_sentence_b_len)]
  ...
  with tf.variable_scope("loss"):
    ...
    logits = tf.reshape(logits, [-1, max_sentence_a_num, max_sentence_b_num])
...
def run_epoch(epoch_no, op_name, sess, training, logits, accuracy, mean_loss, train_opt=tf.constant(0)):
    n_updates = 0
    t_loss = 0
    n_all = 0
    t0 = time()
    ...
    try:
        while True:
            n_updates += 1
            batch_logits, batch_loss, _ , accur = sess.run([logits, mean_loss, train_opt, accuracy], feed_dict={training:True})
            n_sample = batch_logits.shape[0]
            n_all += n_sample
            t_loss += batch_loss * n_sample
            if n_updates%2000 == 0:
                tf.logging.info("epoch: %i  n_update %d , %s: Mins Used: %.2f, Loss: %.4f, Accuarcy: %.2f" %
                                (epoch_no, n_updates, op_name, (time() - t0) / 60.0, t_loss / n_all, 100 * accur))
    
    except tf.errors.OutOfRangeError:
        tf.logging.info("epoch: %i %s: Mins Used: %.2f, Loss: %.4f, Accuarcy: %.2f" %
                        (epoch_no, op_name, (time() - t0)/60.0, t_loss / n_all, 100*accur))
        pass
...
best_score = 0.0
def run_eval(epoch_no, dir_path, op_name, sess, training, accuracy, prob, pair_ids):
    ...
    try:
        ...
        out_path = os.path.join(dir_path, "output_epoch_{}.txt".format(epoch_no))
...
def main(_):
    ...
    text_a_id, text_b_id, input_ids, input_mask, segment_ids, labels = iterator.get_next()  # output dir
    pair_ids = [text_a_id, text_b_id, labels]


    training = tf.placeholder(tf.bool)
    mean_loss, logits, probabilities, accuracy, model = create_model(bert_config = bert_config,
    ...
    if FLAGS.do_train:
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(FLAGS.num_train_epochs):
                ...
                run_eval(epoch, root_path, "valid", sess, training, accuracy, probabilities, pair_ids)
