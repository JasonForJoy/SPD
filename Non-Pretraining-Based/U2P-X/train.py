...
tf.flags.DEFINE_integer("max_utter_num", 8, "max utterance number")
tf.flags.DEFINE_integer("max_utter_len", 20, "max utterance length")
tf.flags.DEFINE_integer("max_profile_num", 5, "max profile number")
tf.flags.DEFINE_integer("max_profile_len", 15, "max profile length")
...
train_dataset = data_helpers.load_dataset(FLAGS.train_file, vocab, FLAGS.max_utter_num, FLAGS.max_utter_len, FLAGS.max_profile_num, FLAGS.max_profile_len)
print('train_pairs: {}'.format(len(train_dataset)))
valid_dataset = data_helpers.load_dataset(FLAGS.valid_file, vocab, FLAGS.max_utter_num, FLAGS.max_utter_len, FLAGS.max_profile_num, FLAGS.max_profile_len)
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        model = MODEL(
            ...
            max_profile_num=FLAGS.max_profile_num,
            max_profile_len=FLAGS.max_profile_len,
        ...
        #     if tvars[i].get_shape().ndims==1:
        #         para_total += int(tvars[i].get_shape()[0])
        #     else:
        #         para_total += int(tvars[i].get_shape()[0])*int(tvars[i].get_shape()[1])
        # print 'Total Parameter Numbers: {}.'.format(para_total)
        # =====================================================================================


        def train_step(x_utterances, x_utterances_len, x_utterances_num, x_profiles, x_profiles_len, x_profiles_num, 
                       x_labels, x_ids, x_utterances_char, x_utterances_char_len, x_profiles_char, x_profiles_char_len):
            """
            ...
            feed_dict = {
              ...
              model.profiles: x_profiles,
              model.profiles_len: x_profiles_len,
              model.profiles_num: x_profiles_num,
              model.target: x_labels,
              model.dropout_keep_prob: FLAGS.dropout_keep_prob,
              model.u_charVec: x_utterances_char,
              model.u_charLen: x_utterances_char_len,
              model.p_charVec: x_profiles_char,
              model.p_charLen: x_profiles_char_len
            }
        ...
        def dev_step():
            ...
            num_test = 0
            num_correct = 0.0
            valid_batches = data_helpers.batch_iter(valid_dataset, FLAGS.batch_size, 1, FLAGS.max_utter_num, FLAGS.max_utter_len, FLAGS.max_profile_num, FLAGS.max_profile_len, charVocab, FLAGS.max_word_length, shuffle=True)
            for valid_batch in valid_batches:
                x_utterances, x_utterances_len, x_utterances_num, x_profiles, x_profiles_len, x_profiles_num, x_labels, x_ids, x_utterances_char, x_utterances_char_len, x_profiles_char, x_profiles_char_len = valid_batch
                feed_dict = {
                  model.utterances: x_utterances,
                  model.utterances_len: x_utterances_len,
                  model.utterances_num: x_utterances_num,
                ...
                for i, prob_score in enumerate(predicted_prob):
                    utterances_id, profiles_id, label = x_ids[i]
                    results[utterances_id].append((profiles_id, label, prob_score))

            # print("Validation Time: {} s".format(time.time() - t0))
        ...
        best_mrr = 0.0
        batches = data_helpers.batch_iter(train_dataset, FLAGS.batch_size, FLAGS.num_epochs, FLAGS.max_utter_num, FLAGS.max_utter_len, FLAGS.max_profile_num, FLAGS.max_profile_len, charVocab, FLAGS.max_word_length, shuffle=True)
        for batch in batches:
            x_utterances, x_utterances_len, x_utterances_num, x_profiles, x_profiles_len, x_profiles_num, x_labels, x_ids, x_utterances_char, x_utterances_char_len, x_profiles_char, x_profiles_char_len = batch
            train_step(x_utterances, x_utterances_len, x_utterances_num, x_profiles, x_profiles_len, x_profiles_num, x_labels, x_ids, x_utterances_char, x_utterances_char_len, x_profiles_char, x_profiles_char_len)
